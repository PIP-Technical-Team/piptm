...
#'   (numeric) and the target variable. When `target_variable` is `NULL`,
#'   `dt` must contain a numeric `welfare` column. This must be a
...
#'
#' @param target_variable Optional character scalar naming the variable on
#'   which statistics should be computed. When `NULL` (default), statistics
#'   are computed on `welfare`.
#'
...
#' )
compute_summary_stats <- function(dt, by = NULL, measures = NULL, target_variable = NULL, grp = NULL) {
...
required_cols <- c(
if (is.null(target_variable)) "welfare" else target_variable,
"weight"
)
...
x <- if (is.null(target_variable)) welfare_v else dt[[target_variable]]
